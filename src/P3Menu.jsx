...
const ITEMS = [
  ...
  { id: "contact", label: "CONTACT",       page: "contact", fontSize: 68, offsetX: 8,  offsetY: 6,  skew: 0,   skewY: -4  },
...
function Clock() {
  ...
  return (
    <div className="p3-clock">
      <div className="p3-clock-time">{hh}<span className="p3-clock-colon">:</span>{mm}<span className="p3-clock-colon">:</span>{ss}</div>
      <div className="p3-clock-date">{date}</div>
    </div>
  );
}
...
export default function P3Menu({ onNavigate }) {
  ...
  const activate = (idx) => { setActive(idx); setAnimKey(k => k + 1); };
    ...
    <>
      <style>{`
        ...
        .p3-shadow-tri {
          ...
          transform: translateY(-40%) translateX(-12px) scaleX(0); transition: transform 0.18s ease;
        }
        .p3-shadow-tri.pop { animation: p3-shadow-pop 0.28s cubic-bezier(0.34,1.56,0.64,1) forwards; }
        ...
        .p3-label-base { font-family: 'Anton', sans-serif; font-style: italic; letter-spacing: 2px; line-height: 0.85; display: block; white-space: nowrap; user-select: none; }
        ...
        .p3-hint-row { display: flex; align-items: center; gap: 8px; font-size: 13px; letter-spacing: 2px; color: rgba(255,255,255,0.28); }
        .p3-hint-key { border: 1px solid rgba(255,255,255,0.2); border-radius: 3px; padding: 1px 6px; font-size: 11px; }
      `}</style>
      ...
      <div className="p3-overlay">
        ...
        <div className={`p3-clock${mounted ? " mounted" : ""}`}>
          <Clock />
        </div>

        <div className={`p3-status${mounted ? " mounted" : ""}`}>
          <div className="p3-status-dot" />
          <span className="p3-status-label">NOW</span>
          <span className="p3-status-value">BUILDING FLIGHTSCOPE</span>
        </div>
          ...
          {ITEMS.map((item, i) => {
            ...
            return (
              <a
                ...
                style={{ marginRight: item.offsetX, marginTop: item.offsetY, transitionDelay: mounted ? `${i * 80}ms` : "0ms" }}
              ...
              >
                ...
                <div className="p3-skew-wrap" style={{ transform: `skewX(${item.skew}deg) skewY(${item.skewY}deg)` }}>
                  ...
                  <div
                    ...
                    style={{ width: estW, height: estH, clipPath: clipFn(estW, estH), transform: `translateY(-50%) scaleX(${isActive ? 1 : 0})` }}
                  />
                  <div className="p3-label-wrap" style={{ opacity }}>
                    ...
                    <span className="p3-label-base p3-label-bright" style={{ fontSize: item.fontSize, clipPath: clipFn(estW, estH) }}>{item.label}</span>
                  </div>
                </div>
              </a>
            );
